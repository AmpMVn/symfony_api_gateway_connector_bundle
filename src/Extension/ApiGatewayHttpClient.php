...
final class ApiGatewayHttpClient implements HttpClientInterface
{
...
//        $session->remove('access_token');
    ...
    public function freshConnect(){
        ...
        $client = $this->clientRegistry
            ->getClient('rentsoft_ms_user_keycloak_api_gateway');

        /** @var Keycloak $provider */
        $provider = $client->getOAuth2Provider();
        $provider->authServerUrl = $this->parameterBag->get('api_gateway_connector')['auth_server_url_intern'];

//        $this->token = $provider->getAccessToken('password', ['username' => $this->parameterBag->get('api_gateway_connector')['user']['username'], 'password' => $this->parameterBag->get('api_gateway_connector')['user']['password']]);
        $this->token = $provider->getAccessToken('client_credentials');
        $this->session->set('access_token', $this->token);
    }
    ...
    public function deserializeCollection(string $className, array $items, int $totalItems) {
        ...
        $array = new ArrayCollection(['data' => $data, 'totalItems' => $totalItems]);
